class Mes {
    ...
    apurarJuros () {
        if (this.totalizador.saldo < 0) {
            ...
            this.totalizador.saldo = this.arredondar(this.totalizador.saldo + this.totalizador.juro);
        } 
    }

    calcularJuros (valor) {
        const juros = this.arredondar(valor * 0.10);
        return juros
    }
    ...
    apurarRendimento () {
        if (this.totalizador.saldo > 0) {
            ...
            this.totalizador.saldo = this.arredondar(this.totalizador.saldo + this.totalizador.rendimento);
        }
    }

    calcularRendimentos (valor) {
        const rendimento = this.arredondar(valor * 0.005);
        return rendimento
    }
}